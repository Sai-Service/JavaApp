...
//    public List<Map> getParkingDetails(Integer ouId, Integer locId, Date fromDate, Date toDate);
    ...
    @Query(value = " SELECT\n"
            ...
            + "    NVL(jc.SRV_ADVISOR, NVL(p.ATTRIBUTE1,'-'))   SERVICE_ADVISOR,\n"
            + "    NVL(jc.SERVICE_TYPE,'-')     SERVICE_TYPE,\n"
            ...
            + "    CASE \n"
            + "        WHEN jc.SERVICE_TYPE IS NOT NULL\n"
            + "        THEN CASE \n"
            + "                 WHEN UPPER(jc.SERVICE_TYPE) LIKE '%BANDP%'\n"
            + "                 THEN 'BODYSHOP' \n"
            + "                 ELSE 'SERVICE' \n"
            + "             END\n"
            + "        ELSE '-' \n"
            ...
            + "               j2.SERVICE_TYPE,\n"
            + "               j2.REG_NO,\n"
            + "               /* Convert both date formats into DATE */\n"
            + "               CASE\n"
            + "                   /* Format 1:\n"
            + "                      DD-MM-YYYY HH24:MI\n"
            + "                      Example: 31-08-2026 10:30\n"
            + "                   */\n"
            + "                   WHEN REGEXP_LIKE(\n"
            + "                            j2.JOB_CARD_DATE_TIME,\n"
            + "                            '^\\d{2}-\\d{2}-\\d{4} \\d{2}:\\d{2}'\n"
            + "                        )\n"
            + "                   THEN TO_DATE(\n"
            + "                            SUBSTR(j2.JOB_CARD_DATE_TIME,1,16)\n"
            + "                            DEFAULT NULL ON CONVERSION ERROR,\n"
            + "                            'DD-MM-YYYY HH24:MI'\n"
            + "                        )\n"
            + "                   /* Format 2:\n"
            + "                      YYYY-MM-DD HH24:MI:SS\n"
            + "                      Example: 2026-08-31 00:00:00\n"
            + "                   */\n"
            + "                   WHEN REGEXP_LIKE(\n"
            + "                            j2.JOB_CARD_DATE_TIME,\n"
            ...
            + "                        )\n"
            + "                   THEN TO_DATE(\n"
            + "                            SUBSTR(j2.JOB_CARD_DATE_TIME,1,19)\n"
            + "                            DEFAULT NULL ON CONVERSION ERROR,\n"
            + "                            'YYYY-MM-DD HH24:MI:SS'\n"
            + "                        )\n"
            + "                   ELSE NULL\n"
