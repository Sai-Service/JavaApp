...
//            + "                      (SELECT *\n"
//            + "          FROM (SELECT trans_segment\n"
//            + "                  FROM gd_fdi_trans\n"
//            + "                 WHERE ge1 = cii.instance_number\n"
//            + "                   AND trans_type = 'WI'\n"
//            + "              ORDER BY trans_date DESC) a\n"
//            + "         WHERE ROWNUM = 1) transSegment,\n"
//            + "       CASE\n"
//            + "          WHEN (SELECT *\n"
//            + "                  FROM (SELECT trans_segment\n"
//            + "                          FROM gd_fdi_trans\n"
...
//    public List<Map> getVehDetailsCsiByRegNo(String regNo);
    ...
    @Query(value = " SELECT cii.instance_number REG_NO, msib.attribute6 chassis_no, \n"
        + "        msib.attribute12 engine_no, cii.install_date registration_date, \n"
        + "        hca.account_number, \n"
        + "        CASE WHEN hp.party_type = 'PERSON' \n"
        + "             THEN hp.person_first_name||' '||hp.person_middle_name||' '||hp.person_last_name \n"
        + "             ELSE party_name END cust_name, \n"
        + "        msib.inventory_item_id, \n"
        + "        (SELECT MAX(variant_cd) FROM gd_fdi_trans \n"
        + "          WHERE ge1 = cii.instance_number AND trans_type = 'WI') variant_code, \n"
        + "        (SELECT fvl.meaning \n"
        + "           FROM (SELECT gd.variant_cd FROM gd_fdi_trans gd \n"
        + "                  WHERE gd.ge1 = cii.instance_number AND gd.trans_type = 'WI' \n"
        + "               ORDER BY gd.trans_date DESC) a, fnd_lookup_values_vl fvl \n"
        + "          WHERE ROWNUM = 1 AND fvl.lookup_code = variant_cd \n"
        + "            AND attribute_category = 'SS_MSIL_SALES_CODE') vehicle_desc, \n"
        + "        '-' service_advisor, '-' transSegment, '-' deptAlloted \n"
        + "   FROM csi_item_instances cii, mtl_system_items_b msib, hz_parties hp, hz_cust_accounts hca \n"
        + "  WHERE cii.instance_number = ?1 \n"
        + "    AND cii.inventory_item_id = msib.inventory_item_id \n"
        + "    AND hp.party_id = cii.owner_party_id \n"
        + "    AND cii.owner_party_account_id = hca.cust_account_id \n"
