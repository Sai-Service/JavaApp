...
//            @Param("salesType") String salesType);
    @Query(value
            ...
            + "    JOIN veh_stock VS\n"
            + "      ON DEMO.CHASSIS_NO = VS.CHASSIS_NO\n"
            + "     AND DEMO.VIN = VS.VIN\n"
            + "    WHERE DEMO.IN_KM IS NOT NULL\n"
            + "    UNION ALL\n"
            + "    SELECT\n"
            + "           TRF.CHASSIS_NO,\n"
            + "           TRF.VIN,\n"
            ...
            + "           TRF.RECD_DATE AS EVENT_DATE,\n"
            + "           'TRF' AS SRC\n"
            + "    FROM SS_DMS_STOCK_TRF TRF\n"
            + "    JOIN veh_stock VS\n"
            + "      ON TRF.CHASSIS_NO = VS.CHASSIS_NO\n"
            + "     AND TRF.VIN = VS.VIN\n"
            + "     AND TRF.ENGINE_NO = VS.ENGINE_NO\n"
            + "    WHERE TRF.RECD_DATE IS NOT NULL\n"
            + "      AND TRF.TOKM IS NOT NULL\n"
            + "),\n"
            + "latest_event AS (\n"
            + "    SELECT\n"
            + "           CHASSIS_NO,\n"
            ...
            + ")\n"
            + "SELECT\n"
            + "       NVL(VS.INSTANCE_NUMBER, '-') AS VEHICLE_NO,\n"
            + "       NVL(VS.CHASSIS_NO, '-') AS CHASSIS_NO,\n"
            + "       NVL(VS.VIN, '-') AS VIN,\n"
            + "       NVL(VS.MODEL_DESC, '-') AS MODEL_DESC,\n"
            + "       NVL(VS.FUEL_DESC, '-') AS FUEL_DESC,\n"
            + "       NVL(VS.VARIANT_DESC, '-') AS VARIANT_DESC,\n"
            + "       NVL(VS.ENGINE_NO, '-') AS ENGINE_NO,\n"
            + "       NVL(VS.VARIANT_CODE, '-') AS VARIANT_CODE,\n"
            + "       NVL(VS.COLOUR, '-') AS COLOUR,\n"
            + "       NVL(VS.COLOUR_CODE, '-') AS COLOUR_CODE,\n"
            + "       NVL(TO_CHAR(LE.EVENT_KM), '0') AS LAST_IN_KM,\n"
            + "       NVL(\n"
            + "           TO_CHAR(\n"
            + "               LE.EVENT_DATE,\n"
            ...
            + "       ON LE.CHASSIS_NO = VS.CHASSIS_NO\n"
            + "      AND LE.VIN = VS.VIN\n"
            + "      AND LE.ENGINE_NO = VS.ENGINE_NO",
            nativeQuery = true)
    public List<Map> getDemoVehDetailsPresentByRegNo(
            @Param("regNo") String regNo,
            @Param("location") String location);
...
//            @Param("salesType") String salesType);
    @Query(value
            ...
            + "      ON DEMO.CHASSIS_NO = VS.CHASSIS_NO\n"
            + "     AND DEMO.VIN = VS.VIN\n"
            + "     AND DEMO.ENGINE_NO = VS.ENGINE_NO\n"
            + "    WHERE DEMO.IN_KM IS NOT NULL\n"
            + "    UNION ALL\n"
            + "    /* TRANSFER EVENTS */\n"
            + "    SELECT\n"
...
//    public Optional<SsDmsVehDemo> findFirstByRegNoOrderByCreationDateDesc(String regNo);
    ...
    @Query(value = "  select ROW_NUMBER() OVER (ORDER BY CREATION_DATE DESC) AS sr_no, NVL(REG_NO,'-') REG_NO, NVL(chassis_no,'-') CHASSIS_NO, NVL(MODEL_DESC,'-') MODEL_DESC, NVL(FUEL_DESC,'-') FUEL_DESC,\n"
            ...
            + "  --NVL(CUST_CONTACT_NO,'-') CUST_CONTACT_NO\n"
            + " NVL(\n"
            + "    RPAD('*', LENGTH(CUST_CONTACT_NO) - 4, '*') ||\n"
            + "    SUBSTR(CUST_CONTACT_NO, -4),\n"
            + "    '-'\n"
            + ") AS CUST_CONTACT_NO ,\n"
            + " NVL(REMARKS,'-') REMARKS,\n"
...
//            @Param("salesType") String salesType);
    ...
    @Query(value = "SELECT STOCK.LOCATION "
            + "FROM MTL_SYSTEM_ITEMS_B MSI "
            + "JOIN CSI_ITEM_INSTANCES CII "
            + "  ON MSI.INVENTORY_ITEM_ID = CII.INVENTORY_ITEM_ID "
            + "JOIN SS_DMS_INV_STOCK STOCK "
            ...
            + "  AND CII.INSTANCE_NUMBER = :regNo",
            nativeQuery = true)
    String getLocationNameByRegNo(@Param("regNo") String regNo);
}
